...
def next_episode(self):
    ...
    self.epsilon = self.initial_epsilon * (1 - self.current_episode /
...
def move_is_legal(self, delta_x, state):
    ...
    collision = check_collision(state['board'], state['stone'], (new_x, state['stone_y']))
...
def move_to_next_stone(self, state):
    ...
    state["next_stone"] = None# reset x and y columns = len(state["board"][0]) stone_length = len(state["stone"][0])
    # state["stone_x"] = int(columns / 2 - stone_length / 2) state["stone_y"] = 0
...
def turn_deltas_to_sequences(self, legal_move_sequences, rotations):
    ...
    for delta in legal_move_sequences:
        ...
        sequences.append(sequence)# If you 're pinned or creating a row
        if len(sequences) == 0:
            sequences = [["CONTINUE"]]
        return sequences
...
def move(self, state, direction):
    ...
    if not check_collision(state["board"], state["stone"], (new_x, state["stone_y"])):
        state["stone_x"] = new_x


def rotate(self, state):
    new_stone = rotate_clockwise(state["stone"])
    if not check_collision(state["board"], new_stone, (state["stone_x"], state["stone_y"])):
